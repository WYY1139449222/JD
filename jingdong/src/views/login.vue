<template>
  <div class="loginPage">
    <header>
      <van-icon name="arrow-left" class="headicon" @click="backFn" />京东注册登录
    </header>
    <section>
      <van-field v-model="username" placeholder="用户名/邮箱/手机号" class="textInp" />
      <van-cell-group>
        <van-field 
        v-model="psw"
        :type="isPsw"
        right-icon="eye-o"
        placeholder="请输入密码"
        @click-right-icon="changePsw"
        class="pswInp"
        >
          <van-button slot="button" size="30px" class="pswBtn">  忘记密码</van-button>
        </van-field>
      </van-cell-group>
    </section>
